...
public class CodeDotnetTool
{
        ...
        [Description("Working directory for the command execution (optional)")] string? workingDirectory = null)
    {
        ...
        try
        {
            var toolConfig = _settings.Tools.Dotnet;
            ...
            else
            {
                ...
                {
                    return $"Error: dotnet executable not found at {executablePath}. Please update config.json with the correct path or leave path empty to use PATH.";
                }
            }
